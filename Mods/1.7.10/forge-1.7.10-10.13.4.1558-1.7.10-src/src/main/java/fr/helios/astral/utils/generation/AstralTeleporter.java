...
public class AstralTeleporter extends Teleporter
{
    ...
    public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotationYaw)
    {
        ...
        else
        {
            for(k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if(this.worldServerInstance.getBlock(k1, i2, l1) == AstralDimension.astralPortail)
                        {
                            while(this.worldServerInstance.getBlock(k1, i2 - 1, l1) == AstralDimension.astralPortail)
                            {
                                --i2;
                            }
        ...
        if(d3 >= 0.0D)
        {
            if(flag)
            { 
                this.destinationCoordinateCache.add(j1, new AstralPortalPosition(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
            ...
            d4 = (double)k + 0.5D;
            int j2 = -1;
            if(this.worldServerInstance.getBlock(i - 1, j, k) == AstralDimension.astralPortail)
            {
                j2 = 2;
            }
            if(this.worldServerInstance.getBlock(i + 1, j, k) == AstralDimension.astralPortail)
            ...
            if(this.worldServerInstance.getBlock(i, j, k - 1) == AstralDimension.astralPortail)
            {
                j2 = 3;
            }
            if(this.worldServerInstance.getBlock(i, j, k + 1) == AstralDimension.astralPortail)
